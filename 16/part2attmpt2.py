...
from input import d
p = "AA" # I hate this so much
#  I thought we were starting from first value
# in part 1 i must have forgotten to change values manually
# and gotten the ocrrect answer
...
# Part 2
# Store best path in 26 minutes for highest pressure
# Store the best path in 26mins for elephant for all opened states
# Take the best human pressure, complement, lookup, add
...
while stack:
    ...
    if True:
        tl = 26 - cur.time
        # cur = cur._replace(time=26, pressure=)
        # print("Optima", cur)
        t = cur.pressure + tl * cur.rate
        m = max(m, t)
        if t > best.get(cur.bool, null).pressure:
            best[cur.bool] = cur._replace(time=26, pressure=t)
        # continue
...
# The path probably never completed all 15 in 26 mins
# h = 0
# for k in best:
#     print(k)
#     pair = mask - k
#     if pair in best:
#         print(best[k], best[pair])
#         h = max(h, best[k].pressure + best[pair].pressure)

h = 0
for k in best:
    for p in best:
        if not (k & p):
            h = max(h, best[k].pressure + best[p].pressure)

...
# start = 'AA'
# # start = 'EV'
# to_visit = [d[start][1].copy()] # The nodes we have to visit
# path = [start]
...
# def simulate(start, n, started):
#     rate, children = d[start]
#     if f := memo.get(start + str(n)):
#         print("hit", start + str(n))
#         return f
...
#     if n == 2:
#         o = [0] + max((simulate(c, 1, started) for c in children), key=lambda s: s[0])
#         if not bitmap[start] & started:
#             o = max([rate, 0], o, key=lambda s: calc(s))
#             if o[0]:
#                 started |= bitmap[start]
#         memo[start + str(n)] = o
#         return o
